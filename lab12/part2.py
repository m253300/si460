...
class Scene:
    def __init__(self, width=600, height=600, caption="Koutrakos - Lab 12", resizable=False, map=[[]]):
        ...
        def calcColor(self, colors):
            return 0.3+(0.7/8)*(len(colors))
            ...
            for i in range(rows-1):
                for j in range(cols-1):
                    ...
                    if not tl in colors.keys():
                        colors[tl] = self.calcColor(colors)

                    # top-right (tr) - (i, j+1)
                    tr = map[i][j+1]
                    if not tr in colors.keys():
                        colors[tr] = self.calcColor(Colors)

                    # bottom-right (br) - (i+1, j+1)
                    br = map[i+1][j+1]
                    if not br in colors.keys():
                        colors[br] = self.calcColor(Colors)

                    # bottom-left (bl) - (i+1, j)
                    bl = map[i+1][j]
                    if not bl in colors.keys():
                        colors[bl] = self.calcColor(Colors)
...
if __name__ == '__main__':
    ...
    myGame = Scene(caption="Koutrakos - Lab 12 - Part 1", map=map)
    pyglet.app.run()
