...
class Particle:
    ...
    #create a function similar to drawDots and wirecube but its the tetrahedron
    # a = side length
    # V1=(0, 0, 0) 
    # V2=(a, 0, 0) 
    # V3=(a/2, (a*sqrt(3))/2, 0) 
    ...
    def drawTetrahedron(self, position, sidelength = 3):
        ...
        glVertex3f(position[0] + sidelength, position[1], position[2])
        glColor3f(0.0, 0.0, 1.0)   # Blue
        glVertex3f(position[0] + (sidelength/2), position[1] + ((sidelength*numpy.sqrt(3))/2), position[2])
        ...
        glVertex3f(position[0] + (sidelength/2), position[1] + ((sidelength*numpy.sqrt(3))/6), position[2] + ((sidelength*numpy.sqrt(6))/3))
